...
const Auth = ({ setUser }) => {
    ...
    const handleRegister = async (e) => {
        ...
        setIsLoading(true);
        setErrors({});
        const { email, newPassword, confirmPassword, firstName, lastName, mobile } = formData;
        ...
        try {
            const userCredential = await createUserWithEmailAndPassword(firebaseAuth, email, newPassword);
            const user = userCredential.user;
    ...
    const handleForgotPassword = async (e) => {
        ...
        try {
            ...
            let errorMessage = 'Failed to send a reset link. Check your email address.';
    ...
    const renderSection = () => {
        switch (currentSection) {
            case 'login':
                return (
                    <div id="login-page" className={`form-section ${currentSection === 'login' ? 'active' : ''}`}>
                        ...
                        <form onSubmit={handleLogin}>
                            ...
                            <div className="input_box">
                                <input type="text" id="login-email" className="input-field" name="email" required onChange={handleInputChange} value={formData.email || ''} />
                            ...
                            <div className="input_box">
                                ...
                                >
                                    {showPassword ? '👁️' : '🔒'} {/* Using emojis for visibility/lock */}
                                </span>
                            </div>
                            <div className="remember-forgot">
                                ...
                                <div className="forgot">
                                    <a onClick={() => handlePageSwitch('forgot-password')}>Forgot password?</a>
                                </div>
                            </div>
                            <div className="input_box">
                            ...
                            <div className="register">
                                <span>Don't have an account? <a onClick={() => handlePageSwitch('signup')}>Register</a></span>
                            </div>
                        </form>
                    </div>
                );
            case 'signup':
                return (
                    <div id="signup-page" className={`form-section ${currentSection === 'signup' ? 'active' : ''}`}>
                         <div className="logo-container">
                            <img src="/logo.jpg" alt="Stylescapes Logo" />
                        </div>
                        <form onSubmit={handleRegister}>
                            ...
                            <div className="name-fields">
                                <div className="input_box" style={{ marginTop: 0 }}>
                                    <input type="text" id="fname" className="input-field" name="firstName" required onChange={handleInputChange} value={formData.firstName || ''} />
                                    <label htmlFor="fname" className="label">First Name</label>
                                </div>
                                <div className="input_box" style={{ marginTop: 0 }}>
                                    <input type="text" id="lname" className="input-field" name="lastName" required onChange={handleInputChange} value={formData.lastName || ''} />
                                    <label htmlFor="lname" className="label">Last Name</label>
                                </div>
                            </div>
                            <div className="input_box">
                                <input type="email" id="signup-email" className="input-field" name="email" required onChange={handleInputChange} value={formData.email || ''} />
                            ...
                            <div className="input_box">
                                <input type="tel" id="signup-mobile" className="input-field" name="mobile" required onChange={handleInputChange} value={formData.mobile || ''} />
                            ...
                            <div className="go-back">
                                <span>Already have an account? <a onClick={() => handlePageSwitch('login')}>Login</a></span>
                            </div>
                        </form>
                    </div>
                );
            ...
            case 'forgot-password':
                return (
                    <div id="forgot-password-page" className={`form-section ${currentSection === 'forgot-password' ? 'active' : ''}`}>
                        <div className="form-content">
                            ...
                            <form onSubmit={handleForgotPassword}>
                                {errors.general && <p className="error-message">{errors.general}</p>}
                                <div className="input_box">
                                    <input type="email" id="forgot-email" className="input-field" name="email" required onChange={handleInputChange} value={formData.email || ''} />
                                ...
                                <div className="go-back">
                                    <span>Remembered your password? <a onClick={() => handlePageSwitch('login')}>Login</a></span>
                                </div>
                            </form>
                        </div>
                    </div>
            ...
            case 'link-sent':
                ...
                return (
                    <div id="link-sent-page" className={`form-section ${currentSection === 'link-sent' ? 'active' : ''}`}>
